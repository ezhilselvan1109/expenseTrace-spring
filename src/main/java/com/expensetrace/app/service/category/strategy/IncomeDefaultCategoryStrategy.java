...
@Service("incomeDefaultStrategy")
@RequiredArgsConstructor
public class IncomeDefaultCategoryStrategy implements DefaultCategoryStrategy {
    ...
    public List<CategoryResponseDto> getAllCategories() {
        ...
        return categoryRepository.findByUserIdAndType(userId,CategoryType.INCOME)
    ...
    public CategoryResponseDto setDefault(UUID categoryId) {
        ...
        if (!CategoryType.INCOME.equals(category.getType())) {
            throw new IllegalArgumentException("Not an income category");
        }

        resetExistingDefaults(userId, CategoryType.INCOME);
    ...
    public CategoryResponseDto getDefault() {
        UUID userId = securityUtil.getAuthenticatedUserId();
        Category category = categoryRepository.findByUserIdAndTypeAndIsDefaultTrue(userId, CategoryType.INCOME)
                .orElseThrow(() -> new ResourceNotFoundException("No default income category"));
        return mapper.toDto(category);
    }
